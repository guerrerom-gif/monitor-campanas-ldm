...
def obtener_datos_meta(dias=30):
    ...
    url = f"https://graph.facebook.com/v19.0/{config.AD_ACCOUNT_ID}/insights"
    params = {
...
def detectar_anomalias(df):
    ...
    idx_completo = pd.MultiIndex.from_product([todas_fechas, todas_horas], names=["fecha", "hora"])
    ...
    merged["lower"] = (merged["mean"] - config.UMBRAL_ANOMALIA * merged["std"]).clip(lower=0)
    merged["upper"] = merged["mean"] + config.UMBRAL_ANOMALIA * merged["std"]
...
def generar_dashboard(df, merged, stats):
    ...
    for _, row in stats.iterrows():
        ...
        hour_stats_list.append({
            ...
            "lo": round(max(0, float(row["mean"]) - config.UMBRAL_ANOMALIA * std), 2),
            "hi": round(float(row["mean"]) + config.UMBRAL_ANOMALIA * std, 2)
        })
    anomalias_list = []
...
<div class="topbar">
  <div class="topbar-left">
    ...
    <div>
      ...
      <div class="topbar-sub" id="lastUpdate">Pollo Campero Guatemala · Actualizado: """ + now + """</div>
    </div>
  </div>
  <div style="display:flex;gap:8px;">
    <span class="tag tag-red" id="tagAnom">""" + str(total_anomalias) + """ anomalías</span>
    <span class="tag tag-green">""" + str(len(dates)) + """ días</span>
    <span class="tag tag-amber">Últimos """ + str(config.DIAS_HISTORICO) + """ días</span>
  </div>
</div>
<div class="main">
  <div class="metrics-row">
    <div class="metric-card"><div class="metric-label">Gasto total</div><div class="metric-value">$""" + f"{total_gasto:,.2f}" + """</div><div class="metric-sub">Últimos """ + str(config.DIAS_HISTORICO) + """ días</div></div>
  ...
  <div class="card">
    <div class="card-header"><div class="card-title">Registro de anomalías detectadas</div><div class="card-sub">Horas donde el consumo cayó más de """ + str(config.UMBRAL_ANOMALIA) + """ desviaciones estándar</div></div>
...
<div class="footer">
  ...
  <span>Metodología: Media ± """ + str(config.UMBRAL_ANOMALIA) + """σ por hora · """ + str(config.DIAS_HISTORICO) + """ días de historial</span>
</div>
<script>
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    datos_raw = obtener_datos_meta(config.DIAS_HISTORICO)
    if datos_raw:
        df = procesar_datos(datos_raw)
        merged, stats = detectar_anomalias(df)
        ruta = generar_dashboard(df, merged, stats)
